...
// CALCULATING INFORMATION
    // Extract orientation of and distance to lines
        void getInfo( vector<Vec4i>* array_lines, Mat result ) {
            // Declarations
            ...
            // Check for valid array
                if( !array_lines->empty() ) {
                    ...
                    // Calculate Distance
                        // Add central line
                            l[0] = 0; l[1] = result.size().height/2; l[2] = result.size().width; l[3] = l[1];
                            array_lines->push_back( l );
                        // Get distance
                }
...
// PROCESSING
    ...
    **
    ** */
    bool imageProcessing( Mat input ) {
        // Declarations
            vector<Vec4i> lines;
            Vec4i l;
...
namedWindow( "xpto1", CV_WINDOW_AUTOSIZE );
imshow( "xpto1", input );
...
namedWindow( "xpto2", CV_WINDOW_AUTOSIZE );
imshow( "xpto2", outputImage );

        // Send information
            getInfo( &lines, outputImage );
waitKey(0);
        // EXIT
            return true;
    }
